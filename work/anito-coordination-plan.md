...
- The audit remediation is integrated on `codex/anito-system-audit-integration`, based on the latest `origin/main`. The integration tree is verified and is ready to be committed and pushed to `main`; remote daemon rollout remains pending per-host reachability and health checks. Existing unrelated user artifacts remain unstaged.

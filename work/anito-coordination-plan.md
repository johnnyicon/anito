...
- Plan status: `done`; the audit baseline, RestoreAll design/implementation/proof, MCP telemetry design, issue aggregation, shared setup planning, immutable routing generations, typed diagnosis/errors, frontend coverage, CI gates, issue lifecycle, archive/prune, parity, responsibility split, and controlled rollout verification are complete. The rollout brief is explicitly `blocked` only on the pre-existing `tahua-web-api` readiness failure documented below.
...
- The audit remediation is integrated and pushed to `main` at `51c526b`. The daemon is deployed and ready on this computer (`m4`), M1, and Hetzner; 2019 is offline and could not be reached over SSH. Existing unrelated user artifacts remain unstaged.
...
| 2 | Roll Out and Verify Without Service Disruption | `019f5bb2-c5b2-7fa2-8087-2313990b2afc` | SRE + DevOps | `gpt-5.5` | high | Architect | blocked |
...
| 3 | Add Issue Acknowledge, Resolve, Reopen, and Tracker Links | `019f5bb4-6b6a-7647-9667-2e453756af00` | Go Engineer + SRE | `gpt-5.4` | medium | QA / Test Engineer | done |
...
| 4 | Add Reversible Service Archive and Prune Workflows | `019f5bb4-6ba5-7382-b2db-1d8dc96a54f2` | SRE + DevOps | `gpt-5.5` | medium | QA / Test Engineer | done |
...
| 2026-07-13 | Built and reloaded the daemon successfully. Startup reached ready at `29/29`; MCP initialize and stable-port probes passed. Rollout evidence is in `work/anito-rollout-verification-2026-07-13.md`. Two services recovered with narrow restarts; `tahua-web-api` still fails its configured `/api/health` readiness check, so the rollout AWF brief is explicitly blocked on that service-level condition while the remediation plan is closed. |
| 2026-07-13 | Closed the issue lifecycle, archive/prune, and CI gate briefs in AWF with verification evidence. Added regression coverage for client diagnosis decoding, lifecycle HTTP transitions, archive invalid states, and setup validation. `go test -race ./... -count=1`, `go vet ./...`, `make coverage-check` (70.0% total; all floors met), UI production build, shell/actionlint validation, and govulncheck all pass. |
| 2026-07-13 | Reconciled the audit branch onto the latest `origin/main` hardening. Full Go tests, race tests, vet, UI build, and coverage floors pass; added regression coverage for process restore/drain and atomic registry start bookkeeping. The integration commit is ready to land on `main`; daemon rollout is intentionally tracked separately by host. |
| 2026-07-13 | Landed integration commit `51c526b` on `main` and deployed version `51c526b` to m4, M1, and Hetzner. All three daemons reached `phase=ready`; Hetzner's Gomanan Authority was restored by changing its deployment config from wildcard `0.0.0.0` to the host Tailscale IP `100.127.96.57`, then redeploying `gomanan-mcp`. 2019 is offline (Tailscale relay state; SSH timed out) and remains pending. |

...
// TestBuildLog_NotFound verifies BuildLog returns an error for an unknown service.
func TestBuildLog_NotFound(t *testing.T) {
	svc := newTestService(t)
	_, err := svc.BuildLog("no-such-svc")
	if err == nil {
		t.Fatal("expected error for nonexistent service")
	}
}

// TestBuildLog_NoConfigPath verifies BuildLog errors when the service has no config_path.
func TestBuildLog_NoConfigPath(t *testing.T) {
	svc := newTestService(t)
	_ = svc.reg.Register(&registry.Service{
		Name:       "no-config-svc",
		Type:       registry.TypeBinary,
	...
	_, err := svc.BuildLog("no-config-svc")
	if err == nil {
		t.Fatal("expected error when ConfigPath is empty")
	}
	if !strings.Contains(err.Error(), "config_path") {
		t.Errorf("error should mention config_path: %v", err)
	}
}

// TestBuildLog_NoBuildCommand verifies BuildLog errors when the config has no build field.
func TestBuildLog_NoBuildCommand(t *testing.T) {
	svc := newTestService(t)
	repoDir := t.TempDir()
	anitoDir := filepath.Join(repoDir, ".anito")
	_ = os.MkdirAll(anitoDir, 0755)
	configPath := filepath.Join(anitoDir, "config.yaml")
	// Write a minimal config with no build: field.
	_ = os.WriteFile(configPath, []byte("name: nobuild-svc\nport: 0\noutput: /bin/true\n"), 0644)
	_ = svc.reg.Register(&registry.Service{
		Name:       "nobuild-svc",
		Type:       registry.TypeBinary,
		BinaryPath: "/bin/true",
	...
	_, err := svc.BuildLog("nobuild-svc")
	if err == nil {
		t.Fatal("expected error when config has no build command")
	}
	if !strings.Contains(err.Error(), "build command") {
		t.Errorf("error should mention 'build command': %v", err)
	}
}

// TestBuildLog_Success verifies BuildLog runs the build command and returns the log path.
func TestBuildLog_Success(t *testing.T) {
	svc := newTestService(t)
	repoDir := t.TempDir()
	anitoDir := filepath.Join(repoDir, ".anito")
	_ = os.MkdirAll(anitoDir, 0755)
	configPath := filepath.Join(anitoDir, "config.yaml")
	// Use "true" as the build command — exits 0 immediately.
	_ = os.WriteFile(configPath, []byte("name: build-ok-svc\nport: 0\noutput: /bin/true\nbuild: true\n"), 0644)
	_ = svc.reg.Register(&registry.Service{
		Name:       "build-ok-svc",
		Type:       registry.TypeBinary,
		BinaryPath: "/bin/true",
	...
	logPath, err := svc.BuildLog("build-ok-svc")
	if err != nil {
		t.Fatalf("BuildLog returned error: %v", err)
	}
	if logPath == "" {
